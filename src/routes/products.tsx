...
function ProductsInner({ data: rawData }: { data: MidClassRow[] }) {
  ...
  // 按粒度生成实际使用的数据集
  const data = useMemo(
    () => granularity === "year" ? aggregateToYear(rawData, qNums) : rawData,
    [rawData, granularity, qNums],
  );
...
function FilterBar(props: {
  ...
  const toggleQN = (n: number) =>
    setQNums(qNums.includes(n) ? qNums.filter((x) => x !== n) : [...qNums, n].sort((a, b) => a - b));
  ...
  const yearLabel = years.length === 0 ? "无"
    ...
    : `${years.length} 年`;
  const qLabel = qNums.length === 4 ? "全部" : qNums.map((n) => `Q${n}`).join(",") || "无";
  ...
  return (
    <Card>
      <CardContent className="flex flex-wrap items-center gap-3 p-3">
        ...
        <SegToggle
          value={granularity}
          options={[{ value: "quarter", label: "按季度" }, { value: "year", label: "按年" }]}
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className="h-8 gap-1.5">
              <Filter className="h-3.5 w-3.5" />季度 ({qLabel})
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-48">
            <div className="mb-2 flex justify-between text-xs">
              <button className="text-primary hover:underline" onClick={() => setQNums([1, 2, 3, 4])}>全选</button>
              <button className="text-muted-foreground hover:underline" onClick={() => setQNums([])}>清空</button>
            </div>
            <div className="grid grid-cols-4 gap-1.5">
              {[1, 2, 3, 4].map((n) => (
                <label key={n} className="flex items-center gap-1 text-xs cursor-pointer">
                  <Checkbox checked={qNums.includes(n)} onCheckedChange={() => toggleQN(n)} />
                  Q{n}
                </label>
              ))}
            </div>
...
// ============ 底部 fst/re/wake 结构对比 ============
function OrderTypeStructure({
  data, regions, quarters, topClasses, classColor,
}: {
  data: MidClassRow[];
  regions: string[]; quarters: string[];
  topClasses: string[]; classColor: Record<string, string>;
}) {
  const scope = data.filter((r) => regions.includes(shortName(r.consume_large_area_name)) && quarters.includes(r.pay_dt_quarter));

  const compute = (k: "order_fst" | "order_re" | "order_wake") => {
    const total = scope.reduce((s, r) => s + (Number(r[k]) || 0), 0);
    const byClass = new Map<string, number>();
    for (const r of scope) {
      byClass.set(r.good_mid_class_merged, (byClass.get(r.good_mid_class_merged) ?? 0) + (Number(r[k]) || 0));
    }
    return topClasses.map((klass) => ({
      klass,
      value: byClass.get(klass) ?? 0,
      share: total ? (byClass.get(klass) ?? 0) / total : 0,
    })).sort((a, b) => b.share - a.share);
  };

  const blocks = [
    { key: "order_fst" as const, label: "首单结构", data: compute("order_fst") },
    { key: "order_re" as const, label: "回购结构", data: compute("order_re") },
    { key: "order_wake" as const, label: "唤醒结构", data: compute("order_wake") },
  ];
  ...
  return (
    <Card>
      ...
      <CardContent className="grid gap-4 md:grid-cols-3">
        {blocks.map((b) => (
          <div key={b.key}>
            <div className="mb-1 text-[11px] text-muted-foreground">{b.label}</div>
            <ResponsiveContainer width="100%" height={Math.max(180, topClasses.length * 22)}>
              <BarChart data={b.data} layout="vertical" margin={{ top: 4, right: 8, bottom: 4, left: 4 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" horizontal={false} />
                <XAxis type="number" tickFormatter={(v: number) => `${Math.round(v * 100)}%`} stroke="var(--muted-foreground)" fontSize={10} domain={[0, "dataMax"]} />
                <YAxis type="category" dataKey="klass" width={70} stroke="var(--muted-foreground)" fontSize={10} />
                <Tooltip contentStyle={tooltipStyle} formatter={(v: unknown) => fmtPct(Number(v))} />
                <Bar dataKey="share">
                  {b.data.map((d) => (
                    <Bar key={d.klass} dataKey="share" fill={classColor[d.klass]} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
...
// ============ 价格结构矩阵 ============
type PriceField = "tag" | "barg";

function PriceMatrixView({
  regions, quarters, cellIndex, topClasses, classColor, metric, topK,
  cellMinW, cellMaxW,
  hoveredClass, setHoveredClass, onClassClick,
}: {
  regions: string[]; quarters: string[];
  cellIndex: Map<string, MidClassRow[]>;
  topClasses: string[]; classColor: Record<string, string>;
  metric: StructMetric; topK: number;
  cellMinW: number; cellMaxW: number;
  hoveredClass: string | null;
  setHoveredClass: (c: string | null) => void;
  onClassClick: (c: string) => void;
  ...
  const [priceField, setPriceField] = useState<PriceField>("tag");

  // 全部可选中类（按当前指标全局聚合排序）
  const allClassesRanked = useMemo(() => {
    const totals = new Map<string, number>();
    for (const reg of regions) {
      for (const q of quarters) {
        const rows = cellIndex.get(`${reg}|${q}`) ?? [];
  ...
  }, [regions, quarters, cellIndex, metric]);

  // 用户自选中类（null = 跟随上方 Top K）
  const [selected, setSelected] = useState<string[] | null>(null);
  const effectiveSelected = selected ?? topClasses;
  const selectedSet = useMemo(() => new Set(effectiveSelected), [effectiveSelected]);

  // 颜色：优先用上方矩阵的色板；自选新增的中类按 PALETTE_20 兜底
  const effectiveColor = useMemo(() => {
    const m: Record<string, string> = { ...classColor };
    let i = 0;
    for (const k of effectiveSelected) {
      if (!m[k]) {
        while (Object.values(m).includes(PALETTE_20[i % PALETTE_20.length])) i++;
        m[k] = PALETTE_20[i % PALETTE_20.length];
        i++;
      }
    }
    return m;
  }, [classColor, effectiveSelected]);

  const toggleClass = (k: string) => {
    const cur = selected ?? topClasses;
    setSelected(cur.includes(k) ? cur.filter((x) => x !== k) : [...cur, k]);
  };

  // 计算单 cell 内要显示的中类（在已选中类里，按当前指标排名，受 topK 上限约束）
  const cellTopClasses = (rows: MidClassRow[]): string[] => {
    const totals = new Map<string, number>();
    for (const r of rows) {
      if (!selectedSet.has(r.good_mid_class_merged)) continue;
      const v = Number(r[metric]) || 0;
      totals.set(r.good_mid_class_merged, (totals.get(r.good_mid_class_merged) ?? 0) + v);
    }
    return [...totals.entries()].sort((a, b) => b[1] - a[1]).slice(0, topK).map(([k]) => k);
  };

  // 全矩阵价格区间（仅含每个 cell 的 Top K 中类）
  const { yMin, yMax } = useMemo(() => {
    let mn = Infinity, mx = -Infinity;
    for (const reg of regions) {
      for (const q of quarters) {
        const rows = cellIndex.get(`${reg}|${q}`) ?? [];
        const top = new Set(cellTopClasses(rows));
        for (const r of rows) {
          if (!top.has(r.good_mid_class_merged)) continue;
          if (priceField === "tag") {
            const lo = Number(r.tag_price_p10), hi = Number(r.tag_price_p90);
            if (lo > 0) mn = Math.min(mn, lo);
            if (hi > 0) mx = Math.max(mx, hi);
          } else {
            const a = Number(r.barg_price_avg), s = Number(r.barg_price_std);
            if (a > 0) { mn = Math.min(mn, a - s); mx = Math.max(mx, a + s); }
          }
        }
      }
    }
    if (!isFinite(mn) || !isFinite(mx) || mx <= mn) return { yMin: 0, yMax: 1 };
    const pad = (mx - mn) * 0.05;
    return { yMin: Math.max(0, mn - pad), yMax: mx + pad };
  }, [regions, quarters, cellIndex, priceField, metric, topK, selectedSet]);

  if (regions.length === 0 || quarters.length === 0) return null;

  const isCustom = selected !== null;
  const selectedLabel = isCustom ? `自选 ${effectiveSelected.length}` : `跟随 Top ${topClasses.length}`;
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-sm">
          区域 × 时间 价格结构矩阵
          <span className="ml-2 text-[11px] font-normal text-muted-foreground">
            每个 cell 取 {isCustom ? "自选中类" : "上方 Top K 中类"} 中按当前指标排名前 {topK} 的箱线图（{priceField === "tag" ? "吊牌价 P10–P90，盒=P25–P75，中线=P50" : "成交价均值 ± 标准差"}）；全矩阵共享 ¥{Math.round(yMin)}–¥{Math.round(yMax)}
          </span>
        </CardTitle>
        <div className="flex items-center gap-2">
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" size="sm" className="h-8 gap-1.5">
                <Filter className="h-3.5 w-3.5" />中类 ({selectedLabel})
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-72">
              <div className="mb-2 flex justify-between text-xs">
                <button className="text-primary hover:underline" onClick={() => setSelected(null)}>跟随上方</button>
                <button className="text-primary hover:underline" onClick={() => setSelected(allClassesRanked)}>全选</button>
                <button className="text-primary hover:underline" onClick={() => setSelected(allClassesRanked.slice(0, topK))}>Top {topK}</button>
                <button className="text-muted-foreground hover:underline" onClick={() => setSelected([])}>清空</button>
              </div>
              <div className="grid grid-cols-1 gap-1 max-h-72 overflow-y-auto">
                {allClassesRanked.map((k) => (
                  <label key={k} className="flex items-center gap-2 text-xs cursor-pointer">
                    <Checkbox
                      checked={effectiveSelected.includes(k)}
                      onCheckedChange={() => toggleClass(k)}
                    />
                    <span className="inline-block h-2.5 w-2.5 rounded-sm" style={{ background: effectiveColor[k] ?? "#ccc" }} />
          ...
          <SegToggle
            ...
            value={priceField}
            options={[{ value: "tag", label: "吊牌价" }, { value: "barg", label: "成交价" }]}
            onChange={(v) => setPriceField(v as PriceField)}
          />
          <SegToggle
            label="方向"
            value={orient}
            options={[{ value: "h", label: "横向" }, { value: "v", label: "纵向" }]}
            onChange={(v) => setOrient(v as "h" | "v")}
          />
        </div>
      </CardHeader>
      <CardContent className="overflow-x-auto p-3 pt-0">
              ...
              {quarters.map((q) => {
                ...
                return (
                  <PriceCell
                    key={q}
                    rows={rows}
                    topClasses={cellTopClasses(rows)}
                    classColor={effectiveColor}
                    yMin={yMin}
                    yMax={yMax}
                    orient={orient}
          ...
          <div className="mt-3 flex flex-wrap gap-2 border-t pt-2">
            {effectiveSelected.map((k) => (
              <div
                ...
                style={{ opacity: hoveredClass && hoveredClass !== k ? 0.4 : 1 }}
              >
                <span className="inline-block h-2.5 w-2.5 rounded-sm" style={{ background: effectiveColor[k] }} />
                {k}
              </div>
            ))}
          </div>
          <div className="mt-2 text-[11px] text-muted-foreground">
            提示：可在右上角「中类」自选要对比的中类（独立于上方 Top K）；hover 中类块或图例可联动高亮。
          </div>
        </div>
      </CardContent>
    </Card>
...
function PriceCell({
  rows, topClasses, classColor, yMin, yMax, orient, priceField,
  hoveredClass, setHoveredClass, onClassClick,
}: {
  rows: MidClassRow[];
  topClasses: string[];
  ...
  const [ref, W] = useElementWidth<HTMLDivElement>(CELL_MIN_W);
  const byClass = new Map<string, MidClassRow>();
  for (const r of rows) byClass.set(r.good_mid_class_merged, r);
  ...
  const slotAxis = (horizontal ? W : H) - inner * 2;
  const valueAxis = (horizontal ? H : W) - inner * 2;
  const n = topClasses.length;
  const slot = slotAxis / Math.max(1, n);
  const boxThick = Math.min(18, slot * 0.55);

  const v2px = (v: number) => {
    const t = (v - yMin) / (yMax - yMin);
    return horizontal ? (inner + valueAxis * (1 - t)) : (inner + valueAxis * t);
  };
    ...
    >
      <svg width={W} height={H} className="block">
        ...
        {(() => {
          ...
          const p = v2px(mid);
          return horizontal
            ? <line x1={inner} x2={W - inner} y1={p} y2={p} stroke="var(--border)" strokeDasharray="2 2" />
            : <line y1={inner} y2={H - inner} x1={p} x2={p} stroke="var(--border)" strokeDasharray="2 2" />;
        })()}
        {topClasses.map((klass, i) => {
          const r = byClass.get(klass);
          if (!r) return null;
          const center = inner + slot * (i + 0.5);
          const dim = hoveredClass && hoveredClass !== klass;
          const color = classColor[klass];
          const opacity = dim ? 0.2 : 1;

          if (priceField === "tag") {
            const p10 = Number(r.tag_price_p10);
            const p25 = Number(r.tag_price_p25);
            const p50 = Number(r.tag_price_p50);
            const p75 = Number(r.tag_price_p75);
            const p90 = Number(r.tag_price_p90);
            if (!p50) return null;
            if (horizontal) {
              const x = center;
              return (
                <g key={klass} opacity={opacity}
                   onMouseEnter={() => setHoveredClass(klass)}
                   onMouseLeave={() => setHoveredClass(null)}
                >
                  <line x1={x} x2={x} y1={v2px(p10)} y2={v2px(p90)} stroke={color} strokeWidth={1} />
                  <line x1={x - boxThick * 0.3} x2={x + boxThick * 0.3} y1={v2px(p10)} y2={v2px(p10)} stroke={color} strokeWidth={1} />
                  <line x1={x - boxThick * 0.3} x2={x + boxThick * 0.3} y1={v2px(p90)} y2={v2px(p90)} stroke={color} strokeWidth={1} />
                  <rect x={x - boxThick / 2} y={v2px(p75)} width={boxThick}
                        height={Math.max(2, v2px(p25) - v2px(p75))}
                        fill={color} fillOpacity={0.4} stroke={color} />
                  <line x1={x - boxThick / 2} x2={x + boxThick / 2} y1={v2px(p50)} y2={v2px(p50)} stroke={color} strokeWidth={2} />
                  <title>{klass}: P10 ¥{Math.round(p10)} / P25 ¥{Math.round(p25)} / P50 ¥{Math.round(p50)} / P75 ¥{Math.round(p75)} / P90 ¥{Math.round(p90)}</title>
                </g>
              );
            } else {
              const y = center;
              return (
                <g key={klass} opacity={opacity}
                   ...
                   onMouseLeave={() => setHoveredClass(null)}
                >
                  <line y1={y} y2={y} x1={v2px(p10)} x2={v2px(p90)} stroke={color} strokeWidth={1} />
                  <line y1={y - boxThick * 0.3} y2={y + boxThick * 0.3} x1={v2px(p10)} x2={v2px(p10)} stroke={color} strokeWidth={1} />
                  <line y1={y - boxThick * 0.3} y2={y + boxThick * 0.3} x1={v2px(p90)} x2={v2px(p90)} stroke={color} strokeWidth={1} />
                  <rect y={y - boxThick / 2} x={v2px(p25)} height={boxThick}
                        width={Math.max(2, v2px(p75) - v2px(p25))}
                        fill={color} fillOpacity={0.4} stroke={color} />
                  <line y1={y - boxThick / 2} y2={y + boxThick / 2} x1={v2px(p50)} x2={v2px(p50)} stroke={color} strokeWidth={2} />
                  <title>{klass}: P10 ¥{Math.round(p10)} / P25 ¥{Math.round(p25)} / P50 ¥{Math.round(p50)} / P75 ¥{Math.round(p75)} / P90 ¥{Math.round(p90)}</title>
                </g>
              );
            }
            ...
            if (horizontal) {
              ...
              return (
                <g key={klass} opacity={opacity}
                   ...
                   onMouseLeave={() => setHoveredClass(null)}
                >
                  <line x1={x} x2={x} y1={v2px(a - s)} y2={v2px(a + s)} stroke={color} strokeWidth={1.5} />
                  <line x1={x - boxThick * 0.4} x2={x + boxThick * 0.4} y1={v2px(a - s)} y2={v2px(a - s)} stroke={color} strokeWidth={1} />
                  <line x1={x - boxThick * 0.4} x2={x + boxThick * 0.4} y1={v2px(a + s)} y2={v2px(a + s)} stroke={color} strokeWidth={1} />
                  <circle cx={x} cy={v2px(a)} r={3} fill={color} />
                  <title>{klass}: 均值 ¥{Math.round(a)} ± {Math.round(s)}</title>
                </g>
              );
            } else {
              const y = center;
              return (
                <g key={klass} opacity={opacity}
                   ...
                   onMouseLeave={() => setHoveredClass(null)}
                >
                  <line y1={y} y2={y} x1={v2px(a - s)} x2={v2px(a + s)} stroke={color} strokeWidth={1.5} />
                  <line y1={y - boxThick * 0.4} y2={y + boxThick * 0.4} x1={v2px(a - s)} x2={v2px(a - s)} stroke={color} strokeWidth={1} />
                  <line y1={y - boxThick * 0.4} y2={y + boxThick * 0.4} x1={v2px(a + s)} x2={v2px(a + s)} stroke={color} strokeWidth={1} />
                  <circle cy={y} cx={v2px(a)} r={3} fill={color} />
                  <title>{klass}: 均值 ¥{Math.round(a)} ± {Math.round(s)}</title>
                </g>
              );
            }
